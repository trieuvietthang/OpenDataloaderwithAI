...
6. [x] **Fix cache model Docling báo tải nhầm + rủi ro quyền ghi khi cài Program Files**:
   - **Nguyên nhân gốc:** khi `docling_models` rỗng, code truyền `artifacts_path=None`, khiến Docling tự chọn cache ẩn ở `~/.cache/huggingface/hub` (không phải thư mục app tự quản lý). Model thực ra ĐÃ có sẵn ở đó, nhưng app luôn kiểm tra nhầm thư mục `docling_models` (luôn rỗng) nên **cứ mỗi lần chạy lại báo "sẽ tải về"** dù không tải lại thật.
   - **Khắc phục:** gọi tường minh `download_models(output_dir=DOCLING_MODELS_DIR)` khi thư mục trống, luôn truyền `artifacts_path` cố định — từ lần thứ 2 trở đi, kiểm tra "đã có model" đúng sự thật, không còn báo nhầm.
...
8. [x] **Đóng gói bản portable (PyInstaller, mang đi máy khác chạy độc lập)**:
   - **Fix `LexGuard.spec`**: `opendataloader-pdf` (chế độ chuyển đổi PDF chuẩn) bundle sẵn 1 file JAR ~24MB truy cập qua `importlib.resources` — PyInstaller không tự gom data file này, phải khai báo tường minh bằng `collect_data_files('opendataloader_pdf')`, nếu không chế độ Standard sẽ báo lỗi thiếu thư viện trên máy khác dù chạy đúng trên máy dev.
   - **Vô hiệu hóa an toàn nút "Cài đặt Docling" trong bản đóng gói**: nút này dùng `sys.executable -m pip install`, nhưng bản đóng gói không có Python/pip thật (`sys.executable` trỏ vào chính `.exe`) — sẽ luôn thất bại. Thêm hằng `IS_FROZEN`; khi đóng gói, banner đổi thành lời giải thích rõ ràng + nút mở hướng dẫn thay vì thử cài rồi lỗi khó hiểu. Docling **không khả dụng trong bản portable** (cần môi trường Python riêng ~2GB) — 3 chế độ còn lại (Standard, Tesseract, AI Vision) hoạt động đầy đủ.
   - Icon `.exe` đổi sang dùng `logo.ico` (đúng định dạng Windows) thay vì `logo.png`.
9. [ ] **Tính năng mới khác cho người dùng**: Ví dụ xuất nhiều định dạng cùng lúc trong 1 lần chạy (đã có sẵn), báo cáo tổng kết theo lô, auto-update, cải thiện installer (`installer.iss`).
